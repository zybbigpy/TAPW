...
def tightbinding_plot_sparsetb(
    ...
    emesh, _, kline, _, _ = mtb.tightbinding_solver(
        n_moire, n_g, n_k, datatype, "0", disp=True, fulltb=True, sparse=True
    )
    n_band = emesh[0].shape[0]
    ...
    ax.set_xticklabels(
        [r"$\bar{K}$", r"$\bar{\Gamma}$", r"$\bar{M}$", r"$\bar{K}^\prime$"]
    )
...
def tightbinding_plot_fulltb(
    ...
    emesh, _, kline, _, _ = mtb.tightbinding_solver(
        n_moire, n_g, n_k, datatype, "0", disp=True, fulltb=True, sparse=False
    )
    n_band = emesh[0].shape[0]
...
def tightbinding_plot_valley_comb(
    ...
    emesh, _, kline, _, _ = mtb.tightbinding_solver(
        n_moire, n_g, n_k, datatype, "valley_comb", disp=True, fulltb=False
    )
    n_band = emesh[0].shape[0]
...
def tightbinding_plot_sep_valley(
    ...
    emesh, _, kline, _, _ = mtb.tightbinding_solver(
        n_moire, n_g, n_k, datatype, "+1", disp=True, fulltb=False
    )
    n_band = emesh[0].shape[0]
