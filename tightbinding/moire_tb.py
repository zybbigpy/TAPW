...
@jit
def _set_g_vec_list(
    m_g_unitvec_1: np.array,
    m_g_unitvec_2: np.array,
    n_g: int,
    n_moire: int,
    valley: int,
) -> np.array:
...
def _set_g_vec_list_comb_valley(
    m_g_unitvec_1: np.array, m_g_unitvec_2: np.array, n_g: int, n_moire: int
) -> np.array:
    """
...
def _set_const_mtrx(
    ...
    gr_mtrx = np.array(
        [
            factor * np.exp(-1j * np.dot(g, r[:2]))
            for g in g_vec_list
            for r in atom_pstn_list
        ]
    ).reshape(n_g, n_atom)
...
def tightbinding_solver(
    ...
    (all_nns, enlarge_atom_pstn_list) = mset.set_atom_neighbour_list(
        atom_pstn_list, m_unitvec_1, m_unitvec_2
    )
    (dr, dd, row, col) = mset.set_relative_dis_ndarray_new(
        atom_pstn_list, enlarge_atom_pstn_list, all_nns
    )

    if disp:
        # k-path sampling
        (ksec, kline, kmesh) = _set_tb_disp_kmesh(
            m_gamma_vec, m_k1_vec, m_k2_vec, m_m_vec, n_k
        )
    else:
        # uniform sampling
    ...
    if valley == "valley_comb":
        g_vec_list = _set_g_vec_list_comb_valley(
            m_g_unitvec_1, m_g_unitvec_2, n_g, n_moire
        )
    elif valley == "+1":
    ...
    (gr_mtrx, tr_mtrx, sr_mtrx) = _set_const_mtrx(
        n_moire,
        dr,
        dd,
        m_g_unitvec_1,
    ...
    (transmat_list, neighbor_map) = _set_kmesh_neighbour(
        n_g, m_g_unitvec_1, m_g_unitvec_2
    )
    ...
    print(
        "atom reciprotocal unit vector".ljust(30),
        ":",
        mset.A_G_UNITVEC_1,
        mset.A_G_UNITVEC_2,
    )
...
def moire_analyze(n_moire: int, n_g: int, valley: int, datatype: str) -> tuple:
    ...
    (ksec, kline, kmesh) = _set_tb_disp_kmesh(
        m_gamma_vec, m_k1_vec, m_k2_vec, m_m_vec, 10
    )
