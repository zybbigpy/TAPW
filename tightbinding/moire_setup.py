...
def _set_moire_angle(n_moire: int) -> float:
    ...
    angle_r = np.arcsin(
        np.sqrt(3) * (2 * n_moire + 1) / (6 * n_moire**2 + 6 * n_moire + 2)
    )
...
def _set_moire(n_moire: int) -> tuple:
    ...
    # moire unit vector
    m_unitvec_1 = (
        -n_moire * A_UNITVEC_1 + (2 * n_moire + 1) * A_UNITVEC_2
    ) @ rt_mtrx_half.T
    m_unitvec_2 = (
        -(2 * n_moire + 1) * A_UNITVEC_1 + (n_moire + 1) * A_UNITVEC_2
    ) @ rt_mtrx_half.T
...
def set_atom_pstn_list(n_moire: int) -> list:
    ...
    small_g_vec = np.array(
        [m_g_unitvec_1, m_g_unitvec_2, -m_g_unitvec_1 - m_g_unitvec_2]
    )
    ...
    for (ix, iy) in product(range(n), range(n)):
        ...
        if (x > -delta) and (x < (1 - delta)) and (y > -delta) and (y < (1 - delta)):
            d = 0.5 * D_LAYER + D1_LAYER * np.sum(
                np.cos(np.dot(small_g_vec, atom_pstn))
            )
...
def set_atom_neighbour_list(
    atom_pstn_list: list,
    m_unitvec_1: np.array,
    m_unitvec_2: np.array,
    distance=2.5113 * A_0,
):
    ...
    enlarge_atom_pstn_list = np.concatenate(
        (atom_pstn_list, area1, area2, area3, area4, area5, area6, area7, area8)
    )
    ...
    all_nns = np.array(
        [
            np.array([idx for idx in nn_indices if idx != i])
            for i, nn_indices in enumerate(ind)
        ],
        dtype=object,
    )
...
def set_relative_dis_ndarray_new(
    atom_pstn_list: list, enlarge_atom_pstn_list: np.array, all_nns: np.array
) -> tuple:
    """
...
def set_relative_dis_ndarray(
    atom_pstn_list: list,
    atom_neighbour_list: list,
    m_g_unitvec_1,
    m_g_unitvec_2,
    m_unitvec_1,
    m_unitvec_2,
    ...
    # construct Rj near Ri list
    atom_neighbour_2darray = np.array(
        [atom_pstn_list[i] for sublist in atom_neighbour_list for i in sublist]
    )

    # (row, col) <=> (index_i, index_j)
    row = [
        iatom
        for iatom in range(len(atom_pstn_list))
        for n in range(len(atom_neighbour_list[iatom]))
    ]
...
def read_atom_pstn_list(n_moire: int, datatype: str) -> list:
    ...
    if datatype == "symm_relax":
        ...
        atom_pstn_list = np.loadtxt(
            "../data/atom/atom" + str(n_moire) + ".csv", delimiter=",", comments="#"
        )
    else:
...
def save_atom_pstn_list(atom_pstn_list: list, path: str, n_moire: int):
    ...
    np.savetxt(
        path + "atom" + str(n_moire) + ".csv", atoms, header="Rx, Ry, d", delimiter=","
    )
