...
def set_rt_mtrx(theta:float):
    ...
    rt_mtrx = np.array([[np.cos(theta), -np.sin(theta), 0],
...
for n_moire in moire_list:
    ...
    for i in range(natoms):
        ...
        new_coord = (atoms[i]+atoms_c31[inds[i][0]]+atoms_c32[inds[i][1]]+atoms_c2x[inds[i][2]]+atoms_c2xc31[inds[i][3]]+atoms_c2xc32[inds[i][4]])/6
        atoms_symm.append(new_coord)
    
    np.savetxt("../data/relaxsymm/symmatom_new"+str(n_moire)+".csv", np.array(atoms_symm), header="Rx, Ry, d", delimiter=',')
    # print("="*50)